...
def INTU_trader(p, df):
    #waiting period
    time.sleep(2)
    #Web scraping
    url = 'https://uk.finance.yahoo.com/quote/INTU?p=INTU'
    ...
    df.loc[df['Mavg_diff'] < -0.1, 'Buy?'] = True
    df.loc[df['Mavg_diff'] > -0.1, 'Buy?'] = False
    df['Strike_Sell'] = price * 1.0005

    #Buy commands/ Stop-loss
    if df['Buy?'].iloc[-1] == True :
        order = api.submit_order(symbol='INTU', qty='1', side='buy', type='market', time_in_force='day', stop_loss={'stop_price':price*0.995})
    if df['Buy?'].iloc[-1] == True:
    ...
    for x in strike_list:
        if x < price:
            ...
            api.order = api.submit_order(symbol='INTU', qty='1', side='sell', type='market', time_in_force='day')
    return df.tail(1)

for _ in ' '*600: print(INTU_trader(p,df))

def liquidate():
    time.sleep(1)
    if float(api.get_position('INTU').unrealized_intraday_pl) > 0.5:
        api.close_all_positions()
    else: 
        return str(api.get_position('INTU').unrealized_intraday_pl) + ' Waiting...'
