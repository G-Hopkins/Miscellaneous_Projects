...
#for _ in ' '*200: print(api.list_positions())
if api.list_positions == []:
        if df['Mavg_diff'].iloc[-1] > 0:
            order = api.submit_order(symbol='AAPL', qty='1', side='buy', type='market', time_in_force='day')
        elif df['Mavg_diff'].iloc[-1] < 0:
            order = order = api.submit_order(symbol='AAPL', qty='1', side='sell', type='market', time_in_force='day')
...
#€#print(yes(1,2))
#api.get_position('AAPL').unrealized_intraday_pl
def who(x):
    x=1
    if api.list_positions() == []:
        if x==1:
